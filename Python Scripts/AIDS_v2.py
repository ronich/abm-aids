...
class Agent:
    ...
    def __init__(self, ID, society):
        ...
        self.inf = True if len(self.society.people) < INF_PCT*N else False
        self.inf_len = 0
        self.known = False
        self.cpl_len = 0
        self.partner = None
...
def Metamodel(points, reps, N, MAX_ITER, RANDOM_SEED):
    ...
    for point in range(points):
        ...
        for rep in range(reps):
            soc = Society(N, MAX_ITER).run()
            values = map(lambda x: str(x), [point, rep, CCEPT_USE, TEST_FREQ, soc])
            print(",".join(values))
